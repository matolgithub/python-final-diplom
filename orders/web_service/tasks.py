...
def send_email(title, message: str, email: str, *args, **kwargs) -> str:
    email_user = environ.get('MAIL_DEFAULT_SENDER', 'matolpydev@gmail.com')
    email_list = list()
    email_list.append(email)
    try:
        msg = EmailMultiAlternatives(subject=title, body=message, from_email=email_user, to=email_list)
        msg.send()
...
def import_shop_data(data, user_id):
    ...
    for item in file['goods']:
        ...
        product_info = ProductInfo.objects.create(product_id=product.id, external_id=item['id'], model=item['model'],
